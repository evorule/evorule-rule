...
> ## ✅ v0.3.0 — 已发布 (2026-09-02)
>
> 本仓库**独立 release**，不绑其他仓的发布节奏；版本号只与本仓 [CHANGELOG](CHANGELOG.md) 对应。
>
> **0.3.0**：双层租户 + 五角色体系；条目查询表达式（服务端过滤）；历史版本快照落库 + 任意版本快照包导出；数据集级 push 事件 schema 声明；knowledge 数据资产化接入（FTS5 trigram 全文检索）；CLI 导入导出子命令；服务目录种子声明文件化（多插件聚合）；`/v1/health` 存活探针；依赖全面走 crates.io 正式版。
>
> **0.2.0**（首次对外发布）：数据治理 MVP + REST API + 认证 + LLM 客户端 + PostgreSQL schema + 密钥管理。
>
> v0.3.0 tag 之后另有若干缺陷修复已合入 main，随下一个补丁版本正式发布——以 [CHANGELOG](CHANGELOG.md) 为准。
>
