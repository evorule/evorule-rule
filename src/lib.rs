//! evorule-rule —— JSON 规则资产治理系统（数据模型层 · 既定设计决策）
//!
//! 对应设计文档历史批次（既定设计决策 数据模型与 schema）与历史批次设计基线。
...
//! - 版本解析：auto_by_effective_date / pinned（设计文档 §4/§5）
//! - 快照包消费层：导出/导入校验/裁剪（历史批次 既定设计决策）
//!
